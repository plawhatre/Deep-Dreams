...
if __name__ == '__main__':
	...
	S = np.expand_dims(S, axis=0) / 255

	# Initiliatize Model for NST
	params = init_param()
	vgg = VGG16(params)
	model = NSTModel(vgg)
	X = tf.Variable(np.random.random((1,224,224,3)).astype('float64'), name='GenImg', trainable=True)
	X, Loss = model.train(X, C, S, weight_s=[0.25/64, 0.25/128, 0.25/256, 0.25/512, 0.25/512])
	I = X.numpy().reshape(224,224,3)
	C = tf.reshape(C,[224,224,3]).numpy()
	S = tf.reshape(S,[224,224,3]).numpy()

	# Initialize model for DeepDream
	I = np.array(I)*255.0
	J = preprocess(I)
	...
	X = tf.Variable(tf.cast(J, tf.float32), name='GenImg', trainable=True)
	model = DeepDreamModel(dream_model, X)
	X = model.train()
	J = deprocess(X).numpy() / 255.0
	I = I / 255.0
	...
	plt.show()
	cv2.imwrite('DeepDream_NST.jpg', J*255)
